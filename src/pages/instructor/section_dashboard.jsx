...
export default function Teacher_Dashboard({ selectedDate }) {
    ...
    useEffect(() => {
        ...
        if (!sectionInfo && sectionId) {
            async function fetchSectionInfo() {
                try {
                    const response = await fetch(`http://localhost/USTP-Student-Attendance-System/instructor_backend/get_section_info.php?section_id=${sectionId}`);
                    if (!response.ok) {
        ...
        const fetchStudents = async () => {
            try {
                ...
                const response = await fetch(
                    `http://localhost/USTP-Student-Attendance-System/instructor_backend/get_students.php?date=${dateStr}&instructor_id=${instructor.instructor_id}&section_id=${sectionId}&course_id=${courseId}&_t=${new Date().getTime()}`
                );
    ...
    const toggleAttendance = async (student) => {
        ...
        try {
            const response = await fetch('http://localhost/USTP-Student-Attendance-System/instructor_backend/save_attendance.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    useEffect(() => {
        const fetchDropdownStudents = async () => {
            try {
                ...
                const response = await fetch(`http://localhost/USTP-Student-Attendance-System/instructor_backend/student_dropdown.php?instructor_id=${instructor.instructor_id}&section_id=${sectionId}&course_id=${courseId}`);
    ...
    const handleAddDropRequest = async () => {
        ...
        try {
            const response = await fetch('http://localhost/USTP-Student-Attendance-System/instructor_backend/add_drop_request.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                                        ...
                                        <div ref={colorModalRef} className="absolute right-0 mt-2 w-79 bg-white p-4 rounded-lg shadow-xl z-50 text-center border border-gray-200">
                                            ...
                                            <div className="flex justify-center gap-4">
                                                <button
                                                    onClick={async () => {
                                                        try {
                                                            const response = await fetch('http://localhost/USTP-Student-Attendance-System/instructor_backend/update_section_color.php', {
                                                                method: 'POST',
                                                                headers: { 'Content-Type': 'application/json' },
                                                                body: JSON.stringify({
                                                                    section_course_id: sectionInfo?.section_course_id,
