...
export default function TeacherDashboard({ selectedDate }) {
    ...
    async function fetchDashboardStats() {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost/ustp-student-attendance-system/api/instructor-backend/GetTeacherDashboardStats.php?instructor_id=${instructor.instructor_id}&date=${dateStr}`
        );
  ...
  // Effect to fetch simplified notifications (from GetRecentReqNotif.php)
  useEffect(() => {
    if (!instructor?.instructor_id) return;

    const fetchSimplifiedNotifications = async () => {
      try {
        const res = await fetch(`http://localhost/ustp-student-attendance-system/api/instructor-backend/GetRecentReqNotif.php?instructor_id=${instructor.instructor_id}`);
  ...
  useEffect(() => {
    const fetchWeeklyAttendance = async () => {
      if (!instructor?.instructor_id) return;
      try {
        const res = await fetch(`http://localhost/ustp-student-attendance-system/api/instructor-backend/GetWeeklyAttendance.php?instructor_id=${instructor.instructor_id}`);
        const data = await res.json();
  ...
  useEffect(() => {
    const fetchSections = async () => {
      if (!instructor?.instructor_id) return;
      try {
        const res = await fetch(`http://localhost/ustp-student-attendance-system/api/instructor-backend/GetSections.php?instructor_id=${instructor.instructor_id}`);
        const data = await res.json();
        setSections(data);
      } catch (err) {
                ...
                {simplifiedNotifications.length > 0 ? (
                  simplifiedNotifications.map(notif => (
                    <li key={notif.id} className="py-2">
                      {/* Assuming GetRecentReqNotif.php returns an object with 'sender' */}
