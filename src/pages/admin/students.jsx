...
export default function Admin_Students() {
    ...
    const fetchStudents = useCallback(async () => { 
        ...
        try {
            ...
            if (Array.isArray(res.data)) {
                ...
                console.error("Unexpected data format from student_api.php:", res.data);
                data = [];
                showMessageModal('Data Error', 'Received unexpected data format for students. Displaying no students.', 'error');
            }
            ...
            showMessageModal('Network Error', 'Failed to fetch students. Please check your network connection or server.', 'error');
        } finally {
            setLoading(false);
        }
    }, [showMessageModal]); 
        ...
        axios.post('http://localhost/USTP-Student-Attendance-System/admin_backend/delete_student.php', {
            ...
            .then((res) => {
                if (res.data.success) {
                    ...
                    showMessageModal('Success!', 'Student deleted successfully.', 'success'); 
                } else {
                    showMessageModal('Deletion Failed', res.data.message || "Failed to delete student.", 'error'); 
                }
            })
            .catch((err) => {
                console.error("Delete error:", err);
                showMessageModal('Error', "An error occurred while deleting the student. Please try again.", 'error'); 
            })
            .finally(() => {
    ...
    const filteredStudents = students.filter((student) => {
        const fullName = `${student.firstname} ${student.middlename || ''} ${student.lastname}`.toLowerCase(); // Added || '' for middlename
