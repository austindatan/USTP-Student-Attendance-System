...
$course_id = isset($data['course_id']) ? (int)$data['course_id'] : 0; // New parameter
$date = isset($data['date']) ? $data['date'] : null;
$lock_status = isset($data['lock_status']) ? (int)$data['lock_status'] : null; // Get lock_status (0 for unlock, 1 for lock)

if (!$section_id || !$course_id || !$date || $lock_status === null) { // Include course_id in check
    echo json_encode(['success' => false, 'message' => 'Missing required fields (section_id, course_id, date, lock_status).']);
    exit;
}

// Update all attendance records for students in this specific section, course, and date
...
$stmt->bind_param("iiis", $lock_status, $section_id, $course_id, $date); // Update bind_param for new integer course_id

if ($stmt->execute()) {
    $action = ($lock_status == 1) ? 'locked' : 'unlocked';
    if ($stmt->affected_rows > 0) {
        echo json_encode(['success' => true, 'message' => 'Attendance ' . $action . ' successfully for section ' . $section_id . ', course ' . $course_id . ' on ' . $date . ' (' . $stmt->affected_rows . ' records updated).']);
    } else {
        echo json_encode(['success' => true, 'message' => 'No existing attendance records found to ' . $action . ' for section ' . $section_id . ', course ' . $course_id . ' on ' . $date . '.']);
    }
} else {
