...
if (json_last_error() !== JSON_ERROR_NONE || $data === null) {
    error_log("JSON Decode Error or null data in link_section_to_course.php: " . json_last_error_msg());
    echo json_encode(["success" => false, "message" => "Invalid JSON format or no data received."]);
    exit();
}

// Extract fields for section_courses
...
if (is_null($section_id) || is_null($course_id) || is_null($schedule_day) ||
    ...
    error_log("Missing required data in link_section_to_course.php. Received: " . print_r($data, true));
    echo json_encode(["success" => false, "message" => "Missing required data to link section to course."]);
    exit();
}

// Prepare and execute insert statement for section_courses
$stmt = $conn->prepare("INSERT INTO section_courses (section_id, course_id, schedule_day, start_time, end_time) VALUES (?, ?, ?, ?, ?)");

if ($stmt === false) {
    error_log("Failed to prepare statement in link_section_to_course.php: " . $conn->error);
