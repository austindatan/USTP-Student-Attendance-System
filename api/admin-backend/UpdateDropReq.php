    ...
    try {
        ...
        if ($original_request_status === 'Dropped') {
            ...
            $stmt_fetch_details_snapshot = $conn->prepare("
                ...
                INNER JOIN SectionCourses sc ON sc.section_course_id = sd.section_course_id
            ...
            $stmt_DeleteStudent_details = $conn->prepare("DELETE FROM student_details WHERE student_details_id = ?");
            if (!$stmt_DeleteStudent_details) {
                throw new Exception("Failed to prepare statement for deleting student details: " . $conn->error);
            }
            $stmt_DeleteStudent_details->bind_param("i", $student_details_id_affected);
            if (!$stmt_DeleteStudent_details->execute()) {
                throw new Exception("Failed to delete student details: " . $stmt_DeleteStudent_details->error);
            }
            $stmt_DeleteStudent_details->close();
